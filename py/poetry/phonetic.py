...
class Accents:
    ...
    def get_accent(self, word0, ud_tags=None):
        ...
        for m2 in (('шы', 'ши'), ('жы', 'жи'), ('цы', 'ци'), ('щю', 'щу'), ('чю', 'чу'), ('цца', 'ться'), ('ща', 'сча')):
            if m2[0] in word:
...
if __name__ == '__main__':
    ...
    for word in 'тишынА сжыгАю соцыАльная магнитозавИсимыми электрощЁточную шестиузловОй двадцатисторОнними двухпротОнными восьмимЕрного пятиствОрчатый тЁрок алЁна самовЫгул нанореснИчками трёхвалЕнтный голубОе дЕтям сочнЕйшего землЕй дождЕй полудождЯми конЕк остаЁтся остаЕтся гОды кИн кинО сЫн'.split():
        n_vowels = 0
        true_stress = -1
        for c in word:
            if c.lower() in 'уеыаоэёяию':
