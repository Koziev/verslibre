...
def predict(input_sentence, model, classes, cached=False, device="cpu"):
    ...
    print(
        "Predictions:",
        ", ".join("{}: {:.4f}".format(c, math.exp(log_prob)) for c, log_prob in zip(classes.values(), log_probs)),
    )
...
def train_discriminator(
    ...
    if dataset == 'CEDR':
        ...
        dataset_data = load_dataset("cedr")
        train_dataset = None
        test_dataset = None
        for part in ['train', 'test']:
            data = dataset_data[part]
            x = []
            y = []
            for row in data:
                text = row['text']
                ...
                try:
                    ...
                    bos_token_id = discriminator.tokenizer.encoder['<s>']

                    if len(seq) < max_length_seq:
                        seq = torch.tensor([bos_token_id] + seq, device=device, dtype=torch.long)
                    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="SST",
        choices=("SST", "clickbait", "toxic", "generic", "CEDR"),
    ...
    tmp_dir = '/home/inkoziev/polygon/text_generator/tmp'
    models_dir = '/home/inkoziev/polygon/text_generator/models'
    data_dir = '/home/inkoziev/polygon/text_generator/data'
